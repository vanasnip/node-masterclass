                    ...
                    _data.delete('checks',checkId,function(err){
                      ...
                      if(checksDeleted == checksToDelete){
                        if(!deletionErrors){
                          ...
                          callback(500, {'Error' : 'Erro ecnountered while attempting to delete all the user\'s cheks. All checks my not have been from the system successfully'});
                        }
                      }
                    })
                  })
...
handlers._tokens.get = function(data,callback){
  ...
  if(id){
    ...
    callback(400,{'Error':'Missing valid id number'});
  }
};
...
// Checks - post 
// Required data: porotcol, url, method,successCodes, timeoutSeconds
// Optional data: none
...
handlers._checks.delete = function(data, callback){
  ...
  if(id){
              ...
              if(!err){
                _data.read('users',checkData.userPhone,function(err,userData){
                  if(!err && userData){
                    ...
                    callback(500,{'Error' : 'Could not find the user who created the check, so could not remove the check from the list of checks onn the user object'});
                  }
                });
    ...
    callback(400,{'Error':'Missing valid field'});
  }
}
